...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchImpactData() {
      try {
        const reports = 10;//await getRecentReports(100);  // Fetch last 100 reports
        const rewards = 10; //await getAllRewards();
        const tasks = 10; //await getWasteCollectionTasks(100);  // Fetch last 100 tasks

        const wasteCollected = 10;
        ...
        const reportsSubmitted = 10; //reports.length;
        const tokensEarned = 10; //rewards.reduce((total, reward) => total + (reward.points || 0), 0);
        const co2Offset = 10; //wasteCollected * 0.5;  // Assuming 0.5 kg CO2 offset per kg of waste
    ...
    <div className={`container mx-auto px-4 py-16 ${poppins.className}`}>
      <section className="text-center mb-20">
        ...
        <h1 className="text-6xl font-bold mb-6 text-gray-800 tracking-tight">
          Taguig City <span className="text-green-600">Waste Management</span>
        </h1>
